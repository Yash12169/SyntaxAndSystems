...
const ThemeController = () => {
  ...
  return (
    <div>
      <div className="btn border-none flex hover:bg-transparent justify-center items-center bg-transparent  w-fit p-4 h-[70px] rounded-[15px]" onClick={()=>setOpenTheme(!openTheme)}>
        <svg
      ...
      {openTheme && (
        <div className="gap-[5rem] absolute " >
          {themes.map((t) => (
          <p key={t} onClick={()=>handleThemeChange2(t)} className="p-3 hover:bg-grey-300">
            {t}
          </p>
        ))}
        </div>
      ...
      <select
        ...
        className="bg-neutral h-[80px] w-[80px] text-primary rounded-[20px] button-ele outline-none"
      >
        {themes.map((t) => (
